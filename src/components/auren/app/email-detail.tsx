  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.key.toLowerCase() === "r" && !e.metaKey && !e.ctrlKey) {
        e.preventDefault();
        onAction?.("Reply to this email");
      }
      if (e.key.toLowerCase() === "s" && !e.metaKey && !e.ctrlKey) {
        e.preventDefault();
        onAction?.("Schedule a 30 minute meeting based on this email");
      }
    };
    document.addEventListener("keydown", handleKeyDown);
    return () => document.removeEventListener("keydown", handleKeyDown);
  }, [onAction, isAgentLoading, email]);

  if (!email) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center bg-white dark:bg-[#383838]">
        <h1 style={{ fontFamily: "var(--font-civane, Georgia, serif)" }} className="text-[36px] text-[rgba(36,27,20,0.35)] dark:text-[rgba(255,255,255,0.35)] tracking-tight">Auren</h1>
        <p className="font-sans text-[13px] text-[rgba(36,27,20,0.35)] dark:text-[rgba(255,255,255,0.35)] mt-2">Select an email to read</p>
      </div>
    );
  }
    ...
    <div className="flex-1 flex flex-col bg-white dark:bg-[#383838] h-full relative overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto px-8 py-8 scrollbar-hide pb-[100px]">
        ...
        <div className="flex items-start justify-between mb-8">
          <h3 style={{ fontFamily: "var(--font-civane, Georgia, serif)" }} className="text-[28px] text-[#241B14] dark:text-[#F4F4F5] tracking-tight leading-snug max-w-2xl">
            {subject}
          </h3>
          ...
          <div className="relative shrink-0" ref={menuRef}>
            <button
              ...
              className="w-8 h-8 rounded-[8px] flex items-center justify-center text-[rgba(36,27,20,0.4)] dark:text-[rgba(255,255,255,0.4)] hover:bg-[rgba(36,27,20,0.04)] dark:bg-[rgba(255,255,255,0.04)] hover:text-[#241B14] dark:text-[#F4F4F5] transition-colors"
              title="More actions"
            >
            ...
            {isMenuOpen && (
              <div className="absolute right-0 top-full mt-1 w-[200px] bg-white dark:bg-[#383838] border border-[rgba(36,27,20,0.08)] dark:border-[rgba(255,255,255,0.08)] rounded-[12px] shadow-[0_8px_30px_rgba(36,27,20,0.1)] z-50 overflow-hidden animate-in fade-in slide-in-from-top-1 duration-150">
                <button
                  onClick={() => handleMenuAction("Create a GitHub issue about this email")}
                  disabled={isAgentLoading}
                  className="w-full flex items-center gap-3 px-4 py-3 font-sans text-[13px] text-[#241B14] dark:text-[#F4F4F5] hover:bg-[rgba(36,27,20,0.03)] dark:bg-[rgba(255,255,255,0.03)] transition-colors disabled:opacity-50 text-left"
                >
                  <GitBranch size={15} className="text-[rgba(36,27,20,0.4)] dark:text-[rgba(255,255,255,0.4)]" />
                  Create GitHub Issue
                </button>
                <div className="h-px bg-[rgba(36,27,20,0.04)] dark:bg-[rgba(255,255,255,0.04)] mx-3" />
                <button
                  onClick={() => handleMenuAction("Archive this email")}
                  disabled={isAgentLoading}
                  className="w-full flex items-center gap-3 px-4 py-3 font-sans text-[13px] text-[#241B14] dark:text-[#F4F4F5] hover:bg-[rgba(36,27,20,0.03)] dark:bg-[rgba(255,255,255,0.03)] transition-colors disabled:opacity-50 text-left"
                >
                  <Archive size={15} className="text-[rgba(36,27,20,0.4)] dark:text-[rgba(255,255,255,0.4)]" />
                  Archive
                </button>
              </div>
            )}
          ...
          {messagesToRender.map((msg, index) => {
            ...
            return (
              <div key={msg.id} className="flex flex-col">
                <div className="flex items-start gap-4 mb-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-baseline justify-between gap-2">
                      <h2 className="font-sans font-bold text-[16px] text-[#241B14] dark:text-[#F4F4F5] truncate">{msg.fromName || msg.from}</h2>
                      <span className="font-sans text-[12px] text-[rgba(36,27,20,0.35)] dark:text-[rgba(255,255,255,0.35)] shrink-0">{new Date(msg.date).toLocaleString()}</span>
                    </div>
                    <div className="font-sans text-[12px] text-[rgba(36,27,20,0.5)] dark:text-[rgba(255,255,255,0.5)] truncate">
                      {msg.from}
                    </div>
                  </div>
                </div>
                ...
                {/* Body */}
                <div 
                  className="font-sans text-[14px] text-[#241B14] dark:text-[#F4F4F5] leading-[1.6] ml-[56px] overflow-hidden"
                >
                ...
                {index < messagesToRender.length - 1 && (
                  <div className="w-full h-[1px] bg-[rgba(36,27,20,0.06)] dark:bg-[rgba(255,255,255,0.06)] mt-8 ml-[56px]" />
                )}
              </div>
            );
          })}
      ...
      {/* Fixed bottom area with gradient fade */}
      <div className="absolute bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-white dark:from-[#2C2C2C] via-white dark:via-[#2C2C2C] to-transparent flex items-center gap-3">
        <button
          onClick={() => onAction?.("Reply to this email")}
          disabled={isAgentLoading}
          className="h-9 pl-4 pr-2 bg-[#E8593C] text-white rounded-[8px] font-sans font-semibold text-[13px] hover:bg-[#D14F31] transition-colors shadow-sm disabled:opacity-50 flex items-center gap-3 group"
        >
